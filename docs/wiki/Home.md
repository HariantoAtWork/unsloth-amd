...
| [Updating Unsloth](Update-Unsloth.md) | Re-run `install.sh` inside `unsloth-amd`; not on the host, not via image rebuild |
...
Operational how-to (compose up, ports, troubleshooting) stays in the root [README](../../README.md). Updating Unsloth is [Update-Unsloth.md](Update-Unsloth.md).
